...
class InterviewSessions(Resource):
    ...
    def post(self):
        ...
        recording_filename = self.__upload_interview_recording(args['recording'])

        interview_session = InterviewSession(
            # We send the UUID generated on the client, so why not use it?
            id=self.interviewSessionID,
            recording_url=recording_filename,
            creator_id=args['creatorID'],
            project_id=args['projectID']
        )

        interview_session.participants.extend(self.__add_participants(args['participants']))
        interview_session.prompts.extend(self.__add_structural_prompts(args['prompts']))
    ...
    def __upload_interview_recording(self, recording):
        """
        ...
        recording_filename = uuid4().hex + ".mp4"
        # TODO: upload to S3
        recording.save(os.path.join(app.config['UPLOAD_FOLDER'], recording_filename))
        return recording_filename

    def __add_participants(self, participants):
        """
        ...
        for p in json.loads(participants):
            ...
            new_participant = InterviewParticipants(
                ...
                interview_id=self.interviewSessionID,
                role=p['Role'])
            _participants_to_add.append(new_participant)
        return _participants_to_add

    def __add_structural_prompts(self, prompts):
        """
